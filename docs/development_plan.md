...
- **任务**:
    ...
    - [x] 引导界面 ，横幅检测不真实、提醒设置 电池管理允许应用完全后台、获取通知权限 应该跳转到 其他特殊权限 页面
...
- **任务**:
    ...
    - [ ] **(增强) 历史记录高级筛选**: 借鉴 `AutoBookKeepingBeta v1.3`，实现按分类、关键字、金额范围、日期、排序等多维度联合查询。
    - [ ] **(增强) 增强数据可视化**: 借鉴 `AutoBookKeepingBeta v1.3`，实现首页饼图的交互式点击，点击图表板块可高亮并联动展示对应账单。
    - [ ] **(新增) 新解析规则支持**: 持续跟进，增加对支付宝"小荷包"等新支付通知格式的解析。
...
- **任务**:
    ...
    - [ ] (可选) **进程隐藏**: 借鉴 `AutoBookKeepingBeta v1.2`，研究并实现将应用在多任务视图中隐藏的选项，作为高级隐私功能。
...
### 第五阶段：长期规划：云同步与多端支持 (经 AutoBookKeepingBeta v1.2 验证的可行方向)
