...
- **任务**:
    ...
    - [ ] **(下一步)** 根据原型，使用Flutter构建所有核心页面的静态UI，包括：
        - [x] 首页 (Dashboard) - *已完成，并已升级为动态数据展示*
        - [x] 账单历史页 (History) - *已创建基础页面*
        - [ ] 手动记账页 (Manual Add)
        - [x] 设置页 (Settings) - *已创建基础页面*
    - [ ] 搭建基础的状态管理框架。
...
- **任务**:
    ...
    - [x] (增强) 实现独立的、可持久化的调试日志系统，用于捕获所有App的通知。
    - [x] (新增) 在调试页面中加入功能强大的通知模拟器，用于测试解析逻辑。
    - [x] (新增) 实现后台记账提醒功能。**（注：现已重构，能正确处理App前后台不同状态）**
    - [x] 编写正则表达式或解析逻辑，用于精确提取微信、支付宝通知中的金额、商户等信息。
    - [x] 实现本地数据库 (`sqflite`) 的集成，完成交易记录的增、删、改、查功能。
    - [x] 对Android真机进行充分测试，确保不同品牌手机的通知格式兼容性。
        - **已解决**: 针对OPPO/一加等ColorOS系统，`NotificationListenerService`无法绑定的问题，最终解决方案为：除了开启"通知读取权限"外，还必须在系统的"最近任务"中对本App进行"上锁"操作，以防止系统后台管理策略将其服务终止。
